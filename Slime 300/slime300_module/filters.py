...
from typing import List, Union, Dict, Tuple, Optional, Any, cast
...
def restore_credits(clip: vs.VideoNode, ref: vs.VideoNode,
    ...
    mask = vdf.mask.Difference().creditless_oped(opstart=opstart, opend=opend, edstart=edstart, edend=edend, **creditless_args)
    mask = mask.std.Convolution([1]*9)
    if show_mask:
        return mask

    return vdf.util.replace_ranges(clip, core.std.MaskedMerge(clip, ref, mask), rng)
